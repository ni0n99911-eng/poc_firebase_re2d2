...
import { getAdminEmails, SITE_CONFIG } from '$lib/modules';
...
function getFromAddress() {
	return env.RESEND_FROM_ADDRESS || 'RE² <onboarding@resend.dev>';
}

/**
 * Send the welcome email to a newly approved user.
 */
 ...
 */
export async function sendNewUserNotification(name: string, email: string) {
	const resend = getResendClient();
	if (!resend) {
