...
export interface Contradiction {
	...
	category: string; // e.g., 'financial', 'market', 'risk', 'concept'
	vision: string; // what the founder said
	reality: string; // what the data shows
	suggestion: string; // how to resolve
}

// Loosely-typed inputs from LaunchPad/frontend data
...
// Map old Anthropic model names to OpenRouter equivalents
const MODEL_MAP: Record<string, string> = {
	'claude-opus-4-1': 'anthropic/claude-sonnet-4',       // downgrade Opus → Sonnet for cost
...
async function callClaudeAPI(
	...
	const telemetryRoute = telemetry?.route ?? 'claude.callClaudeAPI';

	// B2-T2: 24h LLM cache. Wraps the OpenRouter call so identical
	// (model + system + user) tuples within the TTL window return the
	// stored response. getCachedOrFetch handles its own telemetry rows
	// (cache_hit=true/false in metadata), so the inner fetcher below
	// emits only error-path telemetry — no duplicate success rows.
	if (!telemetry?.bypassCache) {
		try {
			const { getCachedOrFetch } = await import('./llm-cache');
	...
	// Legacy direct-call path — used only when cache is bypassed or wrapper failed.
	...
	try {
		...
		if (!response.ok) {
			// B2-0.2: log HTTP errors as telemetry (fire-and-forget, non-blocking)
			try {
				const { logAiCall } = await import('./telemetry');
				logAiCall({
					route: telemetryRoute,
					model: openRouterModel,
			...
			} catch { /* telemetry never blocks the request */ }
		...
		// B2-0.2: emit success telemetry with token counts when OpenRouter reports them.
		// OpenRouter's response shape: data.usage = { prompt_tokens, completion_tokens, total_tokens }
		try {
			const { logAiCall } = await import('./telemetry');
...
/**
 * B2-T2: raw OpenRouter call used by the cache fetcher.
 *
 * Identical to the legacy callClaudeAPI body but factored out so the
 * cache wrapper can call it on miss. Returns { content, tokensIn, tokensOut }
 * so getCachedOrFetch can persist the token counts.
 *
 * Telemetry: only emits ERROR-path rows here. Success-path telemetry is
 * emitted by getCachedOrFetch (with cache_hit=false in metadata) so we
 * never double-log the same call.
 */
async function callOpenRouterRaw(
	...
	telemetry?: { userId?: string | null; sessionId?: string | null; metadata?: Record<string, unknown> }
...
export async function generateLocationNarrative(
...
`;
	...
	try {
		...
		// Return a default business plan structure on error
		return {
			title: `Business Plan for ${category} at ${address}`,
			executiveSummary:
				'Unable to generate business plan at this time. Please try again later.',
			marketAnalysis: '',
...
// ─────────────────────────────────────────────────────
// Compare Mode — rank and explain multiple addresses
// ─────────────────────────────────────────────────────

export interface CompareAddress {
	label: string;          // User-provided label, e.g. "Option A" or street address
	lat: number;
	lng: number;
	locationIQ: number;     // composite Location IQ (0-100)
	visionIQ: number;       // Vision IQ (0-100)
	fitIQ: number;          // Fit IQ (0-100)
	indexScores: {
		transit: number;
		demographics: number;
		competition: number;
...
export interface CompareResult {
	/** Plain-English recommendation paragraph (2-4 sentences). No rent, no Yelp. */
	recommendation: string;
	/** Rank order of addresses (best → worst). 1-indexed. */
	ranking: number[];
	/** One-line reason per address (same order as input addresses array) */
	reasons: string[];
	/** The single standout strength of the top-ranked address */
	topStrength: string;
	/** The single biggest risk of the top-ranked address */
	topRisk: string;
}

/**
 * Generate a plain-English comparison recommendation for 2-5 locations.
 *
 * Accepts pre-computed scores — does NOT make any external API calls.
 * Returns structured recommendation with ranking and per-address reasons.
 */
export async function generateCompareRecommendation(
...
Return ONLY a JSON object with this exact structure (no markdown, no explanation outside the JSON):
{
  ...
  "topRisk": "<single biggest risk of the top-ranked address>"
	...
	try {
		parsed = JSON.parse(cleaned) as CompareResult;
	} catch {
		// Fallback: extract JSON from response if wrapped in other text
		const match = cleaned.match(/\{[\s\S]*\}/);
		if (match) {
			parsed = JSON.parse(match[0]) as CompareResult;
		} else {
...
// ── Vision Anchor AI Research ──
// Researches market demand, trends, and opportunity signals for a concept in a specific area
export interface VisionAnchorResearch {
	trends: Array<{ title: string; insight: string }>;
	demandSignal: string;
	differentiatorTip: string;
}

export async function generateVisionAnchorResearch(
	concept: string,
	conceptLabel: string,
	neighborhood: string,
	borough: string,
...
Area median household income: $${Math.round(medianIncome / 1000)}K
...
Return ONLY a JSON object:
{
  ...
  "differentiatorTip": "<1 sentence: what would make this concept stand out HERE>"
	...
	const raw = await callClaudeAPI(systemPrompt, userMessage, 'claude-3-5-sonnet-20241022');
	const cleaned = raw.replace(/^```(?:json)?\s*/i, '').replace(/\s*```\s*$/i, '').trim();
	let parsed: VisionAnchorResearch;
	try {
...
// ── Fit Improvement AI Recommendations ──
// Generates intelligent, concept/borough/neighborhood-specific fit improvement tips
export interface FitImprovementRec {
	recommendations: Array<{ action: string; impact: string; priority: 'high' | 'medium' | 'low' }>;
	overallAdvice: string;
}

export async function generateFitImprovementRecs(
	concept: string,
	conceptLabel: string,
	neighborhood: string,
	borough: string,
...
- Reference the median income — "$${Math.round(medianIncome / 1000)}K HHI means your avg check of $${avgCheck} is..."
...
Area median HHI: $${Math.round(medianIncome / 1000)}K
...
// Identifies which neighborhoods in a borough are attractive for a given concept
export interface NeighborhoodAttractiveness {
	topNeighborhoods: Array<{ name: string; whyGood: string; avgRent: string }>;
	currentAreaVerdict: string;
}

export async function generateNeighborhoodAttractiveness(
	concept: string,
	conceptLabel: string,
	borough: string,
	...
	medianIncome: number
): Promise<NeighborhoodAttractiveness> {
	const systemPrompt = `You are RE², an AI co-pilot for first-time retail and food & beverage founders in NYC.
...
Return ONLY a JSON object:
{
  ...
  "currentAreaVerdict": "<1-2 sentences comparing current choice to alternatives>"
	...
	const raw = await callClaudeAPI(systemPrompt, userMessage, 'claude-3-5-sonnet-20241022');
	const cleaned = raw.replace(/^```(?:json)?\s*/i, '').replace(/\s*```\s*$/i, '').trim();
	let parsed: NeighborhoodAttractiveness;
	try {
		parsed = JSON.parse(cleaned);
	} catch {
		const match = cleaned.match(/\{[\s\S]*\}/);
...
// ── Business Case AI Recommendations ──
// Uses ALL collected data to generate 3 intelligent recommendations: revenue, cost, positioning
export interface BusinessCaseRec {
	revenue: { title: string; body: string };
	cost: { title: string; body: string };
	positioning: { title: string; body: string };
}

export async function generateBusinessCaseRecs(
	concept: string,
	conceptLabel: string,
	neighborhood: string,
	borough: string,
...
LOCATION INTEL:
- Median HHI: $${Math.round(medianIncome / 1000)}K
