/**
 * Checklist CoPilot — COPILOT-CK-01
 *
 * POST /api/copilot/checklist
 *
 * Knowledge-powered CoPilot for the Launch Checklist page.
 * Draws on condensed NYC CRE knowledge (95 books → 7 files → structured prompt)
 * to help first-time founders navigate leases, permits, regulations, and costs.
 *
...
interface ChecklistCopilotRequest {
	action: 'ask_question' | 'explain_task' | 'what_should_i_know';
	question?: string;
	taskId?: string;
	taskName?: string;
	context: {
...
// ══════════════════════════════════════════════════════════════════════════
// SYSTEM PROMPT — Condensed from 7 knowledge files (~5,000 lines → ~3,500 tokens)
// Sources: Ground Truth Rules, Regulatory Overlay, Broker Knowledge Index,
//          Legal Baseline, Engine Integration Plan, First-Time Buyer Checklist,
//          Ground Truth NYC CRE Sources (130+ verified references)
// ══════════════════════════════════════════════════════════════════════════
...
const PHASE_GREETINGS: Record<string, string> = {
	...
	design:    'Buildout is where budgets get real. I can help with cost estimates, permit timelines, and what to negotiate for TI allowances.',
	...
	finance:   'Financing and insurance can be confusing. I can walk you through SBA loans, what insurance you actually need, and how to budget for the unexpected.',
	ops:       'Operations setup is about systems, staff, and suppliers. I can help you think through what to prioritize before opening.',
	marketing: 'Pre-launch marketing is where excitement meets reality. I can help you plan a realistic launch strategy.',
	opening:   'Almost there! I can help with final inspections, insurance verification, CRT filing, and everything you need before opening day.',
};

// ── Phase-Specific Suggested Prompts ──
...
function buildUserContext(ctx: ChecklistCopilotRequest['context']): string {
...
async function handleAskQuestion(
	question: string,
	ctx: ChecklistCopilotRequest['context']
): Promise<{ message: string; suggestedPrompts?: string[] }> {
...
Answer using your NYC CRE knowledge. Be specific with numbers, timelines, and costs. If the question relates to their specific business type or location, tailor the answer. If you don't know the current/exact answer, say so.`;
	...
	// Return 3 follow-up prompts from the current phase
...
async function handleExplainTask(
	taskName: string,
	ctx: ChecklistCopilotRequest['context']
): Promise<{ message: string; suggestedPrompts?: string[] }> {
	const userContext = buildUserContext(ctx);
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		const body = (await request.json()) as ChecklistCopilotRequest;
		...
		let result: { message: string; suggestedPrompts?: string[] };
