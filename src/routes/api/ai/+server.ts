...
export const POST: RequestHandler = async ({ request, platform }) => {
	...
	try {
		...
		const processSingleRequest = async (body: AIRequest) => {
			if (!body.action) {
				return {
					success: false,
			...
			switch (body.action) {
				case 'narrative': {
					...
					result = await generateLocationNarrative(body.intelReport, body.founderProfile, scores.locationScore, scores.alignmentScore, body.categoryConfig);
					break;
				}
				case 'contradictions': {
					...
					result = await detectContradictions(body.founderProfile, body.intelReport, scores.locationScore, scores.alignmentScore);
					break;
				}
				case 'business-plan': {
					...
					result = await generateBusinessPlan(body.founderProfile, body.intelReport, scores, body.categoryConfig);
					break;
				}
				case 'compare': {
				...
				case 'vision-anchor': {
					...
					result = await generateVisionAnchorResearch(concept, label, body.neighborhood || '', body.borough || '', body.differentiators || '', body.targetClients || '', body.competitors || 0, body.medianIncome || 0);
					break;
				}
				case 'fit-improvement': {
					const concept = body.conceptType || 'specialty_coffee';
					const label = body.conceptLabel || formatConcept(concept);
					result = await generateFitImprovementRecs(concept, label, body.neighborhood || '', body.borough || '', body.fitIQ || 0, body.locationIQ || 0, body.visionIQ || 0, body.sixScores || {}, body.monthlyRent || 0, body.avgCheck || 0, body.competitors || 0, body.differentiators || '', body.medianIncome || 0);
					break;
				}
				case 'neighborhood-attractiveness': {
					const concept = body.conceptType || 'specialty_coffee';
					const label = body.conceptLabel || formatConcept(concept);
					result = await generateNeighborhoodAttractiveness(concept, label, body.borough || 'Manhattan', body.neighborhood || '', body.locationIQ || 0, body.fitIQ || 0, body.transitScore || 0, body.demographicsScore || 0, body.vibrancyScore || 0, body.medianIncome || 0);
					break;
				}
				case 'business-case-recs': {
					const concept = body.conceptType || 'specialty_coffee';
					const label = body.conceptLabel || formatConcept(concept);
					result = await generateBusinessCaseRecs(concept, label, body.neighborhood || '', body.borough || '', body.address || '', body.locationIQ || 0, body.fitIQ || 0, body.visionIQ || 0, body.sixScores || {}, body.dailyCust || 0, body.avgCheck || 0, body.annualRevenue || 0, body.monthlyRent || 0, body.cogsPct || 0, body.laborCost || 0, body.preTaxProfit || 0, body.marginPct || 0, body.rentToRevenuePct || 0, body.medianIncome || 0, body.walkScore || 0, body.competitors || 0, body.crimeScore || 0, body.differentiators || '', body.targetClients || '', body.foodProgram || '', body.storeType || '');
					break;
				}
				default: {
					return { success: false, error: `Invalid action '${body.action}'. Must be one of: 'narrative', 'contradictions', 'business-plan', 'compare', 'vision-anchor', 'fit-improvement', 'neighborhood-attractiveness', 'business-case-recs'` };
				}
			}
		...
		if (Array.isArray(body)) {
			...
			if (!result.success) {
				return json(result as AIResponse, { status: 400 });
			}
			return json(result as AIResponse, { status: 200 });
		}
	} catch (error) {
