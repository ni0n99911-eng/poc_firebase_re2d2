/**
 * API Source Validation Endpoint
 *
 * GET /api/validate-sources?lat=40.7128&lng=-74.0060&type=cafe&address=123+Main+St
 *
 * Runs every API source individually and returns field-level quality data:
 * - Did it return data? (status, response time, size)
 * - What fields came back populated vs null/empty?
 * - What's the data quality? (plausibility checks)
 * - Reliability classification per field
 *
 * For batch validation across multiple locations:
 * POST /api/validate-sources
 * Body: { locations: [{lat, lng, address, type}], businessType: "cafe" }
 */

import type { RequestHandler } from '@sveltejs/kit';
import { fetchCensusData } from '$lib/intel/census';
import { fetchCensusHousing } from '$lib/intel/census-housing';
import { fetchWalkScore } from '$lib/intel/walkscore';
import { fetchInspections } from '$lib/intel/nyc-inspections';
import { fetchCrimeData } from '$lib/intel/nyc-crime';
import { fetchNearbyPlaces, fetchMarketDensity } from '$lib/intel/google-places';
import { scanCompetitors } from '$lib/intel/overpass';
import { fetchLPCData } from '$lib/intel/nyc-lpc';
import { fetchMTARidership } from '$lib/intel/mta-ridership';
import { fetchDCALicenses } from '$lib/intel/dca-licenses';
import { fetchDOBData } from '$lib/intel/dob-permits';
import { fetch311Complaints } from '$lib/intel/nyc-311';
import { fetchPedestrianCounts } from '$lib/intel/nyc-pedestrian';
import { fetchPLUTOData } from '$lib/intel/pluto';
import { fetchSidewalkCafes } from '$lib/intel/sidewalk-cafes';
import { fetchLiquorLicenses } from '$lib/intel/liquor-licenses';
import { fetchFoursquareData } from '$lib/intel/foursquare';
import { fetchYelpData } from '$lib/intel/yelp';
import { fetchMomentumData } from '$lib/intel/momentum';

// ── Field-level quality definitions ──────────────────────────────────
// For each source: which fields are REAL, ESTIMATED, COMPUTED, or HARDCODED
// and what plausibility checks to run

interface FieldAudit {
	field: string;
	value: unknown;
	populated: boolean;
	reliability: 'REAL_API' | 'COMPUTED' | 'ESTIMATED' | 'HARDCODED' | 'STUB';
	plausible: boolean;
	note?: string;
}
...
interface SourceValidation {
	...
	coveragePct: number;
	fields: FieldAudit[];
	inferenceCapabilities: string[];  // What business inferences can this source support?
	crossSourceLinks: string[];       // Which other sources can validate/augment this?
	rawDataSample?: unknown;          // First few items of array fields
}

function auditField(field: string, value: unknown, reliability: FieldAudit['reliability'], plausibilityCheck?: (v: unknown) => boolean, note?: string): FieldAudit {
	const populated = value !== null && value !== undefined && value !== '' && value !== 0 &&
		!(Array.isArray(value) && value.length === 0);
	const plausible = populated ? (plausibilityCheck ? plausibilityCheck(value) : true) : true;
	return { field, value: summarizeValue(value), populated, reliability, plausible, note: note || (plausible ? undefined : 'FAILED PLAUSIBILITY CHECK') };
}
...
async function validateCensus(lat: number, lng: number): Promise<SourceValidation> {
	...
	try {
		...
		const fields = [
			auditField('medianHouseholdIncome', data.medianHouseholdIncome, 'REAL_API', v => typeof v === 'number' && (v as number) > 0 && (v as number) < 500000),
			auditField('totalPopulation', data.totalPopulation, 'REAL_API', v => typeof v === 'number' && (v as number) > 0),
			auditField('populationDensity', data.populationDensity, 'COMPUTED', v => typeof v === 'number' && (v as number) > 0, 'Derived from totalPop / estimated area (~0.3-1.5 sq mi)'),
			auditField('medianAge', data.medianAge, 'REAL_API', v => typeof v === 'number' && (v as number) > 15 && (v as number) < 90),
			auditField('bachelorsPlusPercent', data.bachelorsPlusPercent, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('commuterPercent', data.commuterPercent, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('daytimePopulationRatio', data.daytimePopulationRatio, 'COMPUTED', v => typeof v === 'number' && (v as number) > 0),
			auditField('tractId', data.tractId, 'REAL_API'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Census Demographics', endpoint: 'api.census.gov/data/2022/acs/acs5',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
		...
		return { source: 'Census Demographics', endpoint: 'api.census.gov/data/2022/acs/acs5', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateCensusHousing(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('medianGrossRent', data.medianGrossRent, 'REAL_API', v => typeof v === 'number' && (v as number) > 0 && (v as number) < 10000),
			auditField('medianMonthlyHousingCost', data.medianMonthlyHousingCost, 'REAL_API', v => typeof v === 'number' && (v as number) > 0),
			auditField('rentBurdenedPct', data.rentBurdenedPct, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('vacancyRate', data.vacancyRate, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('ownerOccupiedPct', data.ownerOccupiedPct, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('renterOccupiedPct', data.renterOccupiedPct, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('totalHousingUnits', data.totalHousingUnits, 'REAL_API', v => typeof v === 'number' && (v as number) > 0),
			auditField('builtBefore1950Pct', data.builtBefore1950Pct, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('builtAfter2010Pct', data.builtAfter2010Pct, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Census Housing', endpoint: 'api.census.gov/data/2022/acs/acs5',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'Census Housing', endpoint: 'api.census.gov/data/2022/acs/acs5', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateWalkScore(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('walkScore', data.walkScore, isEstimated ? 'ESTIMATED' : 'REAL_API', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100, isEstimated ? 'Estimated from Overpass POI count' : undefined),
			auditField('transitScore', data.transitScore, isEstimated ? 'ESTIMATED' : 'REAL_API', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('bikeScore', data.bikeScore, isEstimated ? 'ESTIMATED' : 'REAL_API', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('source', data.source, 'HARDCODED'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'WalkScore', endpoint: 'api.walkscore.com/score',
		...
		return { source: 'WalkScore', endpoint: 'api.walkscore.com/score', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateInspections(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('totalNearby', data.totalNearby, 'REAL_API', v => typeof v === 'number' && (v as number) >= 0),
			...
			auditField('avgScore', data.avgScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) < 100),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'NYC Inspections', endpoint: 'data.cityofnewyork.us/43nn-pn8j',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'NYC Inspections', endpoint: 'data.cityofnewyork.us/43nn-pn8j', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateCrime(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('totalCount', data.totalCount, 'REAL_API', v => typeof v === 'number' && (v as number) >= 0),
			auditField('violentCount', data.violentCount, 'COMPUTED'),
			auditField('propertyCount', data.propertyCount, 'COMPUTED'),
			auditField('crimeScore', data.crimeScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'NYPD Crime', endpoint: 'data.cityofnewyork.us/5uac-w243',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'NYPD Crime', endpoint: 'data.cityofnewyork.us/5uac-w243', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateGooglePlaces(lat: number, lng: number, type: string): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('avgRating', data.avgRating, isOverpass ? 'STUB' : 'REAL_API', v => typeof v === 'number' && (v as number) > 0 && (v as number) <= 5, isOverpass ? 'No ratings from Overpass fallback' : undefined),
			auditField('avgPriceLevel', data.avgPriceLevel, isOverpass ? 'STUB' : 'REAL_API', undefined, isOverpass ? 'No price data from Overpass' : undefined),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Google Places', endpoint: isOverpass ? 'overpass-api.de (fallback)' : 'maps.googleapis.com/nearbysearch',
			...
			rawDataSample: data.places?.slice(0, 3).map((p: { name: string; rating: number; priceLevel: number; distance: number; types: string[] }) => ({ name: p.name, rating: p.rating, priceLevel: p.priceLevel, distance: p.distance, types: p.types?.slice(0, 3) })),
		};
	} catch (e) {
		return { source: 'Google Places', endpoint: 'maps.googleapis.com/nearbysearch', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateMarketDensity(lat: number, lng: number, type: string): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('commercialVitality', data.commercialVitality, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Market Density', endpoint: 'maps.googleapis.com (8 category scans)',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
			...
			rawDataSample: data.categories?.map((c: { category: string; count: number; avgRating: number; chainPct: number }) => ({ category: c.category, count: c.count, avgRating: c.avgRating, chainPct: c.chainPct })),
		};
	} catch (e) {
		return { source: 'Market Density', endpoint: 'maps.googleapis.com', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateOverpass(lat: number, lng: number, type: string): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('chainCount', data.chainCount, 'COMPUTED'),
			auditField('independentCount', data.independentCount, 'COMPUTED'),
			auditField('saturationScore', data.saturationScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Overpass (Competitors)', endpoint: 'overpass-api.de',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'Overpass (Competitors)', endpoint: 'overpass-api.de', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateMTA(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('totalDailyRidership', data.totalDailyRidership, 'ESTIMATED', v => typeof v === 'number' && (v as number) > 0, 'ESTIMATED from route count heuristic, NOT real ridership data'),
			auditField('avgDailyRidership', data.avgDailyRidership, 'ESTIMATED', undefined, 'Derived from estimated totalDailyRidership'),
			auditField('peakHourRatio', data.peakHourRatio, 'HARDCODED', undefined, 'Fixed at 0.40 — NYC standard assumption'),
			auditField('transitScore', data.transitScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('stations', data.stations, 'REAL_API'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'MTA Ridership', endpoint: 'data.ny.gov/5f5g-n3cz',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'MTA Ridership', endpoint: 'data.ny.gov/5f5g-n3cz', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateLPC(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'NYC Landmarks', endpoint: 'data.cityofnewyork.us/buis-pvji',
		...
		return { source: 'NYC Landmarks', endpoint: 'data.cityofnewyork.us/buis-pvji', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateDCA(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('newBusinessRate', data.newBusinessRate, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('businessDensity', data.businessDensity, 'COMPUTED'),
			auditField('ecosystemScore', data.ecosystemScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'DCA Licenses', endpoint: 'data.cityofnewyork.us/w7w3-xahh',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'DCA Licenses', endpoint: 'data.cityofnewyork.us/w7w3-xahh', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateDOB(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('developmentScore', data.developmentScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('activeViolationCount', data.activeViolationCount, 'REAL_API', undefined, 'Violations query currently returns empty (geo filter broken)'),
			auditField('riskScore', data.riskScore, 'COMPUTED'),
			auditField('permits', data.permits, 'REAL_API'),
			auditField('violations', data.violations, 'STUB', undefined, 'DOB violations dataset geo query not working — returns empty'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'DOB Permits', endpoint: 'data.cityofnewyork.us/ipu4-2q9a',
		...
		return { source: 'DOB Permits', endpoint: 'data.cityofnewyork.us/ipu4-2q9a', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validate311(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('qualityScore', data.qualityScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('topTypes', data.topTypes, 'COMPUTED'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: '311 Complaints', endpoint: 'data.cityofnewyork.us/erm2-nwe9',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: '311 Complaints', endpoint: 'data.cityofnewyork.us/erm2-nwe9', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validatePedestrian(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('totalPedestrians', data.totalPedestrians, 'REAL_API', v => typeof v === 'number' && (v as number) >= 0),
			auditField('avgAMCount', data.avgAMCount, 'ESTIMATED', undefined, 'AM/PM split is assumed 45%/55%, not real measured data'),
			auditField('avgPMCount', data.avgPMCount, 'ESTIMATED'),
			auditField('peakRatio', data.peakRatio, 'ESTIMATED'),
			auditField('footTrafficScore', data.footTrafficScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('countLocationCount', data.countLocationCount, 'REAL_API'),
			auditField('counts', data.counts, 'REAL_API', undefined, 'Data is borough-approximated, not address-precise'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Pedestrian Counts', endpoint: 'data.cityofnewyork.us/7ym2-wayt',
		...
		return { source: 'Pedestrian Counts', endpoint: 'data.cityofnewyork.us/7ym2-wayt', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validatePLUTO(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('developmentPotential', data.developmentPotential, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('lots', data.lots, 'REAL_API'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'PLUTO Tax Lots', endpoint: 'data.cityofnewyork.us/64uk-42ks',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'PLUTO Tax Lots', endpoint: 'data.cityofnewyork.us/64uk-42ks', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateSidewalkCafes(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('vibrancySignal', data.vibrancySignal, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('typeBreakdown', data.typeBreakdown, 'COMPUTED'),
		];

		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Sidewalk Cafes', endpoint: 'data.cityofnewyork.us/qcdj-rwhu',
			status: populated >= 3 ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'Sidewalk Cafes', endpoint: 'data.cityofnewyork.us/qcdj-rwhu', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateLiquor(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('nightlifeDensity', data.nightlifeDensity, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('vibrancySignal', data.vibrancySignal, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
			auditField('typeBreakdown', data.typeBreakdown, 'COMPUTED'),
		];
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Liquor Licenses', endpoint: 'data.ny.gov/9s3h-dpkz',
			status: populated === fields.length ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
		...
		return { source: 'Liquor Licenses', endpoint: 'data.ny.gov/9s3h-dpkz', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateFoursquare(lat: number, lng: number, type: string): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('chainCount', data.chainCount, 'COMPUTED'),
			auditField('independentCount', data.independentCount, 'COMPUTED'),
			auditField('avgPopularity', data.avgPopularity, 'REAL_API', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 1, 'Foursquare proprietary popularity metric'),
			auditField('highTrafficVenues', data.highTrafficVenues, 'COMPUTED'),
			auditField('peakTrafficScore', data.peakTrafficScore, 'COMPUTED'),
			auditField('avgRating', data.avgRating, 'REAL_API', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 10, 'Foursquare uses 1-10 scale'),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Foursquare', endpoint: 'places-api.foursquare.com',
		...
		return { source: 'Foursquare', endpoint: 'places-api.foursquare.com', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateYelp(lat: number, lng: number, type: string): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			...
			auditField('avgRating', data.avgRating, 'REAL_API', v => typeof v === 'number' && (v as number) > 0 && (v as number) <= 5),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Yelp', endpoint: 'api.yelp.com/v3/businesses/search',
			status: populated >= 7 ? 'SUCCESS' : 'PARTIAL',
			responseTimeMs: ms, fieldCount: fields.length, populatedCount: populated,
			coveragePct: Math.round((populated / fields.length) * 100),
			fields,
			...
			rawDataSample: data.businesses?.slice(0, 3).map((b: { name: string; rating: number; reviewCount: number; priceLevel: number; distance: number; primaryCategory: string }) => ({ name: b.name, rating: b.rating, reviews: b.reviewCount, price: b.priceLevel, dist: Math.round(b.distance), cat: b.primaryCategory })),
		};
	} catch (e) {
		return { source: 'Yelp', endpoint: 'api.yelp.com/v3/businesses/search', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}

async function validateMomentum(lat: number, lng: number): Promise<SourceValidation> {
	const start = Date.now();
	try {
		...
		const fields = [
			auditField('compositeScore', data.compositeScore, 'COMPUTED', v => typeof v === 'number' && (v as number) >= 0 && (v as number) <= 100),
		...
		const populated = fields.filter(f => f.populated).length;
		return {
			source: 'Momentum', endpoint: 'data.cityofnewyork.us (DOB + DCA + 311)',
		...
		return { source: 'Momentum', endpoint: 'data.cityofnewyork.us', status: 'FAILED', responseTimeMs: Date.now() - start, fieldCount: 0, populatedCount: 0, coveragePct: 0, fields: [auditField('error', (e as Error).message, 'STUB')], inferenceCapabilities: [], crossSourceLinks: [] };
	}
}
...
export const GET: RequestHandler = async ({ url }) => {
	...
	const sources = results.map(r => r.status === 'fulfilled' ? r.value : {
		source: 'UNKNOWN', endpoint: 'UNKNOWN', status: 'FAILED' as const,
		responseTimeMs: 0, fieldCount: 0, populatedCount: 0, coveragePct: 0,
		fields: [auditField('error', (r.reason as Error)?.message || 'Unknown', 'STUB')],
		inferenceCapabilities: [], crossSourceLinks: [],
	});
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const locations: { lat: number; lng: number; address: string; type?: string }[] = body.locations || [];
	...
	// Run each location sequentially to avoid hammering APIs
	const allResults = [];
	for (const loc of locations) {
		const url = new URL(`http://localhost/api/validate-sources?lat=${loc.lat}&lng=${loc.lng}&type=${loc.type || defaultType}&address=${encodeURIComponent(loc.address || '')}`);
		...
		const sources = results.map(r => r.status === 'fulfilled' ? r.value : {
			source: 'UNKNOWN', endpoint: 'UNKNOWN', status: 'FAILED' as const,
			responseTimeMs: 0, fieldCount: 0, populatedCount: 0, coveragePct: 0,
			fields: [], inferenceCapabilities: [], crossSourceLinks: [],
		});
