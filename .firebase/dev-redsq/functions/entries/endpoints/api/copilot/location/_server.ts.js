...
import { d as db, a as blockGroupScores, f as blockGroupIntel, c as copilotConversations } from "../../../../../chunks/db-server.js";
...
import { e as extractEditorialSignal } from "../../../../../chunks/editorial-signals.js";
import { g as getDoc09Intelligence } from "../../../../../chunks/conceptKPIs.js";
import { a as fitGrade, f as fitTierLabel, c as fitNextTier } from "../../../../../chunks/decision-engine.js";
function buildLocationConceptKnowledge(bizType) {
...
async function loadLocationContext(geoid) {
  let visionData = null;
  try {
  ...
  try {
    ...
    }).from(blockGroupScores).where(eq(blockGroupScores.geoid, geoid));
  } catch (e) {
  ...
  try {
    intelResData = await db.select({
      source: blockGroupIntel.source,
      data: blockGroupIntel.data
    }).from(blockGroupIntel).where(eq(blockGroupIntel.geoid, geoid));
  } catch (e) {
...
function buildDataSummary(ctx, businessType) {
  const parts = [];
  ...
  const visionIQ = visionIQKeys.length > 0 ? ctx.scores[visionIQKeys.find((k) => k.includes(":")) || visionIQKeys[0]] : null;
  if (visionIQ) {
...
async function handleInitialAnalysis(ctx, launchpad) {
  const dataSummary = buildDataSummary(ctx, launchpad?.businessType);
  const locIQv2 = ctx.scores["location_iq_v2"];
  locIQv2?.score ?? ctx.scores["location_iq"]?.score ?? 0;
  locIQv2?.components?.grade ?? ctx.scores["location_iq"]?.components?.grade ?? "N/A";
...
async function handleExplainScore(ctx, scoreType, launchpad) {
  const dataSummary = buildDataSummary(ctx, launchpad?.businessType);
  const scoreKeyMap = {
...
async function handleImproveFit(ctx, launchpad, geoid, scoringContext) {
  const visionPct = typeof scoringContext?.visionCompletionPct === "number" ? scoringContext.visionCompletionPct : null;
...
You're currently at ${filledApprox} of 12 Vision inputs — filling three more flips the score from PRELIMINARY to PARTIAL and gives the Brain enough signal to recalibrate this location to your specific business model.`,
  ...
  return {
    ...
    suggestedPrompts: missing.length > 1 ? [`Skip this — what's my current score?`, `Why does ${fieldToAsk} matter?`] : void 0
  };
}
async function handleAskQuestion(ctx, question, launchpad, scoringContext) {
  const dataSummary = buildDataSummary(ctx, launchpad?.businessType);
  ...
  if (scoringContext?.address) {
    ...
    grade = scoringContext.grade ?? (fitForDerivation > 0 ? fitGrade(fitForDerivation) : void 0);
    tier = scoringContext.tier ?? (fitForDerivation > 0 ? fitTierLabel(fitForDerivation) : void 0);
    nextTier = scoringContext.nextTier !== void 0 ? scoringContext.nextTier : fitForDerivation > 0 ? fitNextTier(fitForDerivation) : void 0;
...
- NEXT TIER: ${nextTier.points} points to ${nextTier.label} (threshold ${nextTier.threshold})` : fitForDerivation > 0 ? "\n- NEXT TIER: already at Strong Path (top tier)" : "";
    liveScoreContext = `
...
You have ${Math.round(maxUpside)} total points of upside left across ${unanswered.length} unanswered ${unanswered.length === 1 ? "input" : "inputs"}.`,
  ...
  if (intent === "improve_levers") {
    const _visionPct = typeof scoringContext?.visionCompletionPct === "number" ? scoringContext.visionCompletionPct : null;
...
${intent === "general" ? `Answer their question using the scores and data above. Be specific, cite the actual numbers from SCORES ON SCREEN. Frame answers in Grade/Tier language when a GRADE / TIER line is present. If the question is about something not in the data, give general advice noting it's not location-specific.

End with 2-3 suggested follow-up questions.` : `Follow the INTENT instructions above exactly. Do not deviate from the required format. Do not append follow-up questions for this intent — the UI already shows chips.`}`;
  const result = await callLLM({
    model: "sonnet",
    system: LOCATION_SYSTEM_PROMPT,
    messages: [{ role: "user", content: userMsg }],
  ...
  return {
    message: result.content,
    suggestedPrompts: intent === "general" && suggestedPrompts.length > 0 ? suggestedPrompts : intent === "general" ? ["Tell me more about the competition here.", "What are the biggest risks?", "How does this area compare to alternatives?"] : void 0
  };
}
...
const POST = async ({ request }) => {
  ...
  try {
    ...
    if (body.churnRiskSnippet) ctx.churnSnippet = body.churnRiskSnippet;
