...
import { a as addressToGeoidWithCandidates } from "../../../../../chunks/_server.js";
const QUESTIONS = [
  // Step 1
  {
    key: "businessType",
...
async function generateAcknowledgment(key, answer, context) {
  try {
...
async function saveToFounderSession(userId, step, answer, context) {
  try {
...
const POST = async ({ request }) => {
  ...
  try {
    ...
    const answer = body.answer.trim().slice(0, 1e3);
    ...
    const response = {
      acknowledgment,
      nextQuestion: isComplete ? "You're all set! Your profile is complete. Ready to explore locations?" : nextQ.question,
