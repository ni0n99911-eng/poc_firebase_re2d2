...
async function scheduleCheckIns(params) {
  ...
  const rows = CHECK_IN_SCHEDULE.map((ci) => ({
    ...
    scheduled_for: new Date(now.getTime() + ci.daysAfter * 24 * 60 * 60 * 1e3).toISOString(),
    status: "pending"
  }));
  try {
    for (const r of rows) {
...
async function sendCheckInEmail(params) {
  ...
  const from = private_env.RESEND_FROM_ADDRESS || "RE² <onboarding@resend.dev>";
  ...
  const gradeColor = params.grade?.startsWith("A") ? "#34d399" : params.grade?.startsWith("B") ? "#fbbf24" : params.grade?.startsWith("C") ? "#f97316" : "#ef4444";
...
export {
  processCheckIns as p,
  scheduleCheckIns as s
};
